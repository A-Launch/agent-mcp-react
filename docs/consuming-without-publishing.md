...
| Route | Works | Why |
|---|---|---|
| **A packed tarball** (`file:…/agent-mcp-react-<version>.tgz`) | **Yes** | The recommended route. Verified end to end by `pnpm verify:consumer` |
...
```bash
# In this repository
pnpm pack                 # → agent-mcp-react-<version>.tgz  (runs `prepare`, so dist/ is always fresh)
```
...
{
  "dependencies": {
    "agent-mcp-react": "file:../agent-mcp-react/agent-mcp-react-<version>.tgz"
  }
}
```
